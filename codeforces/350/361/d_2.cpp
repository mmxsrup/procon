...
public:
	...
	segtree_max(int n_): n(n_){ //n_要素数
		n = 1;
		while(n < n_) n *= 2;
		// rep(i, 2 * n + 1)dat[i] = -INF; //(1) 初期値を最小に
	}
...
public:
	...
	segtree_min(int n_): n(n_){ //n_要素数
		n = 1;
		while(n < n_) n *= 2;
		// rep(i, 2 * n + 1)dat[i] = INF; //(1) 初期値を最大に
	}
	void update(int k, T val){ // k番目の値(0-indexed)を val に変更
		k += n - 1; //葉の節点
		dat[k] = val;
...
int main(void){
	...
	// rep(i, n) sega.dat[n + i - 1] = a[i];
	// REP(i, n - 1) sega.dat[i] = max(sega.dat[i * 2 + 1], sega.dat[i * 2 + 2]);
	rep(i, n) segb.update(i, b[i]);
	// rep(i, n) segb.dat[n + i - 1] = b[i];
	// REP(i, n - 1) segb.dat[i] = min(segb.dat[i * 2 + 1], segb.dat[i * 2 + 2]);
